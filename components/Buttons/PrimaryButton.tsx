    ...
    <div className={`relative inline-block min-w-[250px] ${classNames?.divContainer}`}>
      ...
      >
        ...
        <span className="relative block">
          {icon && (
            <motion.div className={`icon absolute right-0 z-[9999] ${classNames?.icon}`} animate={iconControls}>
              {icon}
            </motion.div>
          )}
          {children}
        ...
        <motion.span
          className={`absolute left-0 top-0 flex h-full w-full items-center justify-center bg-[var(--primary-300)] text-xl text-white ${classNames?.span}`}
