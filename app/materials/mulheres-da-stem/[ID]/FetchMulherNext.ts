...
export default async function FetchMulherNext(ID: string) {
  ...
  const guriaUrl = `${env.SITE_URL}/api/gurias?guria=${decodedName}`
...
export async function fetchGuriaImage(ID: string) {
  const decodedName = decodeURIComponent(ID)
  const guriaUrl = `${env.SITE_URL}/api/gurias?guria=${decodedName}&image=true`
...
export async function fetchTags() {
  const tagsUrl = `${env.SITE_URL}/api/tags`
