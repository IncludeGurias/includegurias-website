  ...
  return (
    ...
    >
      <div className="section mt-28 flex w-full flex-col items-center gap-2">
        ...
        />
        <div
          className={`flex max-w-6xl gap-4 border-2 border-transparent pl-4 ${
            selectedTags ? "h-125px" : "h-[0px]"
          } my-4`}
          style={{ transition: "all ease-in-out 0.5s" }}
        >
          {selectedTags && selectedTags.length > 0 && (
            <Reveal className="flex w-full flex-row items-center gap-4 border-2 px-8 py-4">
              <Text fontSize="xl" fontWeight="bold" color="white" alignSelf="center">
                Tags
              </Text>
                ...
                {filteredGurias.length > 0 ? (
                  <>
                    {filteredGurias.map((womanData: womanType, index: number) => (
                      <Reveal delay={index === 0 || index % 4 === 0 ? 0 : 0.1 * (index % 4)} key={womanData.name}>
                        <WomanCard
                          key={womanData.name}
                          name={womanData.name}
